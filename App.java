...
class App {
    public static void login() {
        ...
        login_but.addActionListener((ActionListener) new ActionListener() { // Perform action

            public void actionPerformed(ActionEvent e) {

                String username = F_user.getText(); // Store username entered by the user in the variable "username"
                String password = String.valueOf(F_pass.getPassword()); // Store password entered by the user in the
                                                                        // variable "password"
                ...
                } else if (password.equals("")) // If password is null
                {
                    ...
                    try {
                        ...
                        stmt.executeUpdate("USE Bibliotheca"); // Use the database with the name "Bibliotheca"
                        ...
                        if (!rs.next()) { // Move pointer below
                            ...
                            f.dispose();
                            // ERROR CAUSED BY THE LINE BELOW as this rs is forward only
    ...
    public static Connection connect() {
        try {
            ...
            Connection con = DriverManager.getConnection("jdbc:mysql://localhost/mysql?user=root&password=Dada@2410");
    ...
    public static void create() {
        try {
            ...
            while (resultSet.next()) {
                ...
                if (databaseName.equals("Bibliotheca")) {
                    // System.out.print("yes");
                    Statement stmt = connection.createStatement();
                    // Drop database if it pre-exists to reset the complete database
                    String sql = "DROP DATABASE Bibliotheca";
                    stmt.executeUpdate(sql);
                }
            }
            Statement stmt = connection.createStatement();

            String sql = "CREATE DATABASE Bibliotheca"; // Create Database
            stmt.executeUpdate(sql);
            stmt.executeUpdate("USE Bibliotheca"); // Use Database
            // Create Users Table
            ...
            // Create Books table
            stmt.executeUpdate(
                    "CREATE TABLE BOOKS(BID INT NOT NULL PRIMARY KEY, BNAME VARCHAR(50), GENRE VARCHAR(20), PRICE INT)");
            // Create Issued Table
            stmt.executeUpdate(
    ...
    public static void user_menu(String UID) {
        ...
        view_but.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                Connection connection = connect();
                String sql = "select * from books where bid not in (select bid from issued where return_date is null);"; // Retreive data from database
                try {
                    Statement stmt = connection.createStatement(); // connect to database
                    stmt.executeUpdate("USE Bibliotheca"); // use librabry
        ...
        my_book.addActionListener(new ActionListener() { // Perform action
            public void actionPerformed(ActionEvent e) {
                ...
                // retrieve data
                String sql = "select issued.iid, books.bid, books.bname,issued.issued_date, issued.return_date, issued.period, issued.fine from issued join books on books.bid = issued.bid where uid="
                        + UID_int;
                try {
                    Statement stmt = connection.createStatement();
                    // use database
                    stmt.executeUpdate("USE Bibliotheca");
                    stmt = connection.createStatement();
    ...
    public static void admin_menu() {
        ...
        JButton delete_but = new JButton("Delete Book");// creating instance of JButton to create or reset database
        delete_but.setBounds(450, 60, 120, 25);// x axis, y axis, width, height
        delete_but.addActionListener(new ActionListener() { // Perform action
            public void actionPerformed(ActionEvent e) {
                ...
                // set labels
                JLabel l1;
                l1 = new JLabel("Book ID(BID)"); // Label 4 for Book ID
                ...
                JButton submit = new JButton("Submit");// creating instance of JButton to submit details
                submit.setBounds(130, 70, 80, 25);// x axis, y axis, width, height
                submit.addActionListener(new ActionListener() {

                    public void actionPerformed(ActionEvent e) {
                        String id = F_bid.getText();
                        ...
                        try {
                            Statement stmt = connection.createStatement();
                            stmt.executeUpdate("USE Bibliotheca");
                            stmt.executeUpdate("DELETE FROM books WHERE BID=" + id_int);
                ...
                g.add(l1);
                g.add(F_bid);
                g.setSize(430, 200);// 400 width and 500 height
        ...
        view_but.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                Connection connection = connect(); // connect to database
                String sql = "select * from books where bid not in (select bid from issued where return_date is null);"; // select all books
                try {
                    Statement stmt = connection.createStatement();
                    stmt.executeUpdate("USE Bibliotheca"); // use database
                    stmt = connection.createStatement();
        ...
        add_book.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                F_bname.setBounds(110, 53, 200, 30);
                ...
                F_price.setBounds(110, 127, 200, 30);

                JButton create_but = new JButton("Submit");// creating instance of JButton to submit details
                create_but.setBounds(130, 157, 80, 25);// x axis, y axis, width, height
                create_but.addActionListener(new ActionListener() {
                    ...
                    public void actionPerformed(ActionEvent e) {
                        ...
                        try {
                            Statement stmt = connection.createStatement();
                            stmt.executeUpdate("USE Bibliotheca");
                            stmt.executeUpdate("INSERT INTO BOOKS(BID, BNAME,GENRE,PRICE) VALUES ('" + id_int + "','"
                                    + bname + "','" + genre
                                    + "'," + price_int + ")");
                ...
                g.add(F_price);
                g.add(F_bid);
                g.setSize(430, 300);// 400 width and 500 height
                g.setLayout(null);// using no layout managers
                g.setVisible(true);// making the frame visible
                g.setLocationRelativeTo(null);
                ...
                create_but.addActionListener(new ActionListener() {
                    ...
                    public void actionPerformed(ActionEvent e) {
                        ...
                        try {
                            ...
                            stmt1.executeUpdate("USE Bibliotheca");
                            ...
                            if (days > diff_int) { // If number of days are more than the period then calculcate fine
